...
### The app
`app/` is the Flutter wallet, with its Rust bridge in `app/rust` (crate `pico`). The bridge
is **its own workspace**, not a member of the picomint workspace — it cross-compiles for
mobile and needs a size-optimized `[profile.release]` that a member could not carry. It
reaches the picomint crates by path, so an API change breaks it in the same commit.

Both bridge trees are generated by `flutter_rust_bridge_codegen generate` and neither is
committed: `app/rust/src/frb_generated.rs` and `app/lib/bridge_generated.dart/`. Nothing in
`app/` compiles until codegen has run once. Root `cargo` commands never touch the bridge;
CI's `app` job runs codegen and then lints it.

Android and iOS binaries are built locally on macOS via `app/build-android.sh` and
`app/build-ios.sh`. CI never builds them.
