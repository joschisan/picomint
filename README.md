...
Four daemons, each with its own manual for the person who runs it:
...
- [**Broker**](docker-broker/README.md) — moves ecash between mints, usually run in the cloud.
...
| `ghcr.io/joschisan/picomint-broker-daemon:main` | The broker, with `picomint-broker-cli` on its `PATH` | [Broker](docker-broker/README.md) |
...
| `picomint-rugpull` | A Linux binary on the [`main-latest`](https://github.com/joschisan/picomint/releases/tag/main-latest) pre-release that drains a decommissioned mint's wallet | [Rugpull](docker-node/README.md#rugpull) |
