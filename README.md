...
Nodes run on a fresh **Ubuntu 26.04 LTS desktop** (amd64) with a screen and keyboard. The node has no web UI: everything from the setup ceremony on happens through the admin CLI, and this section is the complete manual for it, written so that an operator, or an agent working for one, can run every step.
...
Download [`docker-node/docker-compose.yml`](docker-node/docker-compose.yml), which runs the node and Bitcoin Core side by side, then pull and start:
