...
//! what happened: the client's event log, the node's consensus events. A
//! trailer task reads that log forward and inserts every entry into
//! `{DATA_DIR}/analytics/analytics.sqlite`, exploded into the rows the
//! embedder's registry derives for it with
