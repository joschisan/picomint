...
pub async fn run_dkg_secp(
    ...
    let mut dkg = DkgSecp::new(num_nodes, identity);
...
mod tests {
    ...
    fn test_dkg_secp() {
        ...
        let mut dkgs = num_nodes
            .node_ids()
            .map(|node| (node, DkgSecp::new(num_nodes, node)))
        ...
        while keys.len() < num_nodes.total() {
            match steps.pop_front().unwrap() {
                (send_node, DkgStepSecp::Broadcast(message)) => {
                ...
                (send_node, DkgStepSecp::Messages(messages)) => {
                ...
                (send_node, DkgStepSecp::Result(step_keys)) => {
