...
impl ConsensusApi {
    ...
    /// the local database so this node can enforce both when serving the
    ...
    /// Set or clear this node's announced expiry status. All
    /// nodes must announce byte-equal values for clients to accept the
