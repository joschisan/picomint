...
fn node_list_section(
    ...
    let total_nodes = connected_nodes.len() + 1;
    let can_start_dkg = mint_size.is_some_and(|expected| total_nodes == expected as usize);

    html! {
        div id="node-list-section" {
            @if let Some(expected) = mint_size {
                span { (format!("{total_nodes} of {expected} nodes connected.")) }
            } @else {
                span { "Add setup code for every other node." }
            }
            ...
            @if !connected_nodes.is_empty() {
                ...
                div style="display: flex; justify-content: center" {
                    button type="button" class="btn btn-danger" style="border: none" onclick="if(confirm('Are you sure you want to reset all nodes?')){document.getElementById('reset-form').submit();}" {
                        "Reset Nodes"
                    }
                }
            }
            ...
            @if can_start_dkg {
                ...
                form id="add-setup-code-form" class="form-stack" hx-post=(ADD_SETUP_CODE_ROUTE) hx-target="#node-list-section" hx-swap="outerHTML" {
                    ...
                    button type="submit" class="btn btn-primary btn-lg btn-block" { "Add Node" }
                }
            }
        }
    }
...
fn setup_form_content(error: Option<&str>) -> Markup {
    html! {
        form id="setup-form" class="form-stack" hx-post=(ROOT_ROUTE) hx-target="#setup-form" hx-swap="outerHTML" {
            input type="text" id="name" name="name" placeholder="Your Node Name" required;

            div class="alert alert-warning" {
                "Exactly one node must set the global config."
            }
                ...
                div class="toggle-content form-stack" {
                    ...
                    div class="field" {
                        span class="field-label" {
                            "Total number of nodes (including you)"
                        }
...
async fn setup_form(State(state): State<Arc<SetupApi>>) -> impl IntoResponse {
    ...
    Html(single_card_layout("Node Setup", setup_form_content(None)).into_string()).into_response()
}
...
async fn post_restore_config(
    ...
    let waiting = html! {
        div class="alert alert-info" {
            "Config restored. The node is rejoining the mint — you'll be redirected once it's back online."
        }
        ...
        span class="hint" style="text-align: center" { "Waiting for node to come online..." }
    };
