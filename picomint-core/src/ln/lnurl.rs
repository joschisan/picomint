...
/// is 8 for the largest supported federation of `3f + 1 = 22`.
pub const MAX_GUARDIANS_PER_LNURL: usize = 8;

/// Payload embedded in the LNURL a client hands out.
///
/// Names no federation id and no gateways. `info` is the hash of the
/// [`FederationInfoResponse`] the daemon expects back, which is what lets a
...
///
/// Every field is immutable for the federation's lifetime, so the string
/// stays valid however often the gateway set turns over.
///
/// [`FederationInfoResponse`]: crate::methods::FederationInfoResponse
